...
def frame(sequence):
    return {"outputs": [{}, {"frame_serial": sequence,
        "est_joint_firc": [216], "est_joint_firc_dot": [10000, -20000, 30000, 4000, -5000, 6000]}]}


class ForceReaderTests(unittest.TestCase):
    def run_reader(self, values, *, clock=None):
        robot = Mock()
        robot.subscribe.side_effect = values
        sdk = SimpleNamespace(DCSS=Mock(), Marvin_Robot=Mock(return_value=robot))
        with tempfile.TemporaryDirectory() as directory, \
                patch.dict("sys.modules", {"SDK_PYTHON.fx_robot": sdk}), \
    ...
    def test_continues_past_ten_frames_until_interrupted_and_releases_connection(self):
        ...
        robot.release_robot.assert_called_once()
